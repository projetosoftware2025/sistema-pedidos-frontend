...
export const HomeView = () => {
  ...
  useEffect(() => {
    ...
    const buscarCategorias = async () => {
      try {
        const response = await axios.get("http://localhost:8080/categoria/buscar-categorias")
    ...
    const buscarProdutos = async () => {
      try {
        const response = await axios.get("http://localhost:8080/produto/buscar-produtos")
        if (response.status === 200) {
